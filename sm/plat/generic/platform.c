...
static void
fw_platform_lookup_special(void* fdt, int root_offset) {
  int pos, noff;
...
static int
generic_nascent_init(void) {
  if (platform_has_mlevel_imsic) imsic_local_irqchip_init();
  return 0;
}

static int
generic_early_init(bool cold_boot) {
...
static int
generic_vendor_ext_check(long extid) {
...
generic_vendor_ext_provider(
    long extid, long funcid, const struct sbi_trap_regs* regs,
    unsigned long* out_value, struct sbi_trap_info* out_trap) {
  if (generic_plat && generic_plat->vendor_ext_provider) {
    return generic_plat->vendor_ext_provider(
        extid, funcid, regs, out_value, out_trap, generic_plat_match);
  }
...
static int
generic_pmu_init(void) {
  return fdt_pmu_setup(fdt_get_address());
}

static uint64_t
generic_pmu_xlate_to_mhpmevent(uint32_t event_idx, uint64_t data) {
  uint64_t evt_val = 0;
  ...
  if (event_idx == SBI_PMU_EVENT_RAW_IDX)
    ...
    if (!evt_val) evt_val = (uint64_t)event_idx;
  }
