...
bool
HSS_Timer_IsElapsed(HSSTicks_t startTick, HSSTicks_t durationInTicks) {
  return (HSS_GetTime() > (startTick + durationInTicks));
}

void
HSS_SpinDelay_MilliSecs(uint32_t milliseconds) {
...
void
HSS_SpinDelay_Secs(uint32_t seconds) {
  HSSTicks_t delayTick = HSS_GetTime();
