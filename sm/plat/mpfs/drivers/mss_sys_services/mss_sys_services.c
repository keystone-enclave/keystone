 ...
 */
static uint16_t
execute_ss_polling_mode(
    uint8_t cmd_opcode, uint8_t* cmd_data, uint16_t cmd_data_size,
    uint8_t* p_response, uint16_t response_size, uint16_t mb_offset,
    uint16_t response_offset);

static uint16_t
execute_ss_interrupt_mode(
    uint8_t cmd_opcode, uint8_t* cmd_data, uint16_t cmd_data_size,
    uint8_t* p_response, uint16_t response_size, uint16_t mb_offset,
    uint16_t response_offset);
...
MSS_SYS_get_serial_number(uint8_t* p_serial_number, uint16_t mb_offset) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_SERIAL_NUMBER_REQUEST_CMD, NULL_BUFFER,
        MSS_SYS_WITHOUT_CMD_DATA, p_serial_number,
        (uint16_t)MSS_SYS_SERIAL_NUMBER_RESP_LEN, mb_offset,
...
MSS_SYS_get_user_code(uint8_t* p_user_code, uint16_t mb_offset) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_USERCODE_REQUEST_CMD, NULL_BUFFER,
        MSS_SYS_WITHOUT_CMD_DATA, p_user_code,
        (uint16_t)MSS_SYS_USERCODE_RESP_LEN, mb_offset,
        MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_USERCODE_REQUEST_CMD, NULL_BUFFER,
...
MSS_SYS_get_design_info(uint8_t* p_design_info, uint16_t mb_offset) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_DESIGN_INFO_REQUEST_CMD, NULL_BUFFER,
        MSS_SYS_WITHOUT_CMD_DATA, p_design_info,
        (uint16_t)MSS_SYS_DESIGN_INFO_RESP_LEN, mb_offset,
        MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_DESIGN_INFO_REQUEST_CMD, NULL_BUFFER,
...
MSS_SYS_get_device_certificate(
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_DEVICE_CERTIFICATE_REQUEST_CMD, NULL_BUFFER,
        MSS_SYS_WITHOUT_CMD_DATA, p_device_certificate,
        (uint16_t)MSS_SYS_DEVICE_CERTIFICATE_RESP_LEN, mb_offset,
        MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_DEVICE_CERTIFICATE_REQUEST_CMD, NULL_BUFFER,
...
MSS_SYS_read_digest(uint8_t* p_digest, uint16_t mb_offset) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_READ_DIGEST_REQUEST_CMD, NULL_BUFFER,
        MSS_SYS_WITHOUT_CMD_DATA, p_digest,
        (uint16_t)MSS_SYS_READ_DIGEST_RESP_LEN, mb_offset,
        MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_READ_DIGEST_REQUEST_CMD, NULL_BUFFER,
...
MSS_SYS_query_security(uint8_t* p_security_locks, uint16_t mb_offset) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_QUERY_SECURITY_REQUEST_CMD, NULL_BUFFER,
        MSS_SYS_WITHOUT_CMD_DATA, buf,
        (uint16_t)(MSS_SYS_QUERY_SECURITY_RESP_LEN + 3u), mb_offset,
        MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_QUERY_SECURITY_REQUEST_CMD, NULL_BUFFER,
...
MSS_SYS_read_debug_info(uint8_t* p_debug_info, uint16_t mb_offset) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_READ_DEBUG_INFO_REQUEST_CMD, NULL_BUFFER,
        MSS_SYS_WITHOUT_CMD_DATA, p_debug_info,
        (uint16_t)MSS_SYS_READ_DEBUG_INFO_RESP_LEN, mb_offset,
        MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_READ_DEBUG_INFO_REQUEST_CMD, NULL_BUFFER,
...
MSS_SYS_read_envm_parameter(uint8_t* p_envm_param, uint16_t mb_offset) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_READ_ENVM_PARAM_REQUEST_CMD, NULL_BUFFER,
        MSS_SYS_WITHOUT_CMD_DATA, p_envm_param,
        (uint16_t)MSS_SYS_READ_ENVM_PARAM_RESP_LEN, mb_offset,
        MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_READ_ENVM_PARAM_REQUEST_CMD, NULL_BUFFER,
...
MSS_SYS_secure_nvm_write(
  ...
  *p_frame = snvm_module; /*SNVMADDR - SNVM module*/
  ...
  if ((format == MSS_SYS_SNVM_AUTHEN_TEXT_REQUEST_CMD) ||
    ...
    if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
      status = execute_ss_interrupt_mode(
          format, &frame[0], (uint16_t)MSS_SYS_AUTHENTICATED_TEXT_DATA_LEN,
          NULL_BUFFER, MSS_SYS_NO_RESPONSE_LEN, mb_offset,
          MSS_SYS_COMMON_RET_OFFSET);
    } else {
      status = execute_ss_polling_mode(
          format, &frame[0], (uint16_t)MSS_SYS_AUTHENTICATED_TEXT_DATA_LEN,
    ...
    if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
      status = execute_ss_interrupt_mode(
          format, &frame[0], (uint16_t)MSS_SYS_NON_AUTHENTICATED_TEXT_DATA_LEN,
          NULL_BUFFER, MSS_SYS_NO_RESPONSE_LEN, mb_offset,
          MSS_SYS_COMMON_RET_OFFSET);
    } else {
      status = execute_ss_polling_mode(
  ...
  *p_frame = snvm_module; /*SNVMADDR - SNVM module*/
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        ...
        (uint16_t)MSS_SYS_SECURE_NVM_READ_DATA_LEN, response, (data_len + 4u),
        mb_offset, (uint16_t)MSS_SYS_SECURE_NVM_READ_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_SNVM_READ_REQUEST_CMD, &frame[0],
        (uint16_t)MSS_SYS_SECURE_NVM_READ_DATA_LEN, response, (data_len + 4u),
...
MSS_SYS_nonce_service(uint8_t* p_nonce, uint16_t mb_offset) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_NONCE_SERVICE_REQUEST_CMD, NULL_BUFFER,
        MSS_SYS_WITHOUT_CMD_DATA, p_nonce,
        (uint16_t)MSS_SYS_NONCE_SERVICE_RESP_LEN, mb_offset,
        MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_NONCE_SERVICE_REQUEST_CMD, NULL_BUFFER,
  ...
  *(uint32_t*)input_data = l_periph_addr;
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        ...
        (uint16_t)MSS_SYS_EXECUTE_UIC_SCRIPT_DATA_LEN, NULL_BUFFER,
...
MSS_SYS_authenticate_bitstream(uint32_t spi_flash_address, uint16_t mb_offset) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        ...
        (uint8_t*)&l_spi_flash_address,
        (uint16_t)MSS_SYS_BITSTREAM_AUTHENTICATE_DATA_LEN, NULL_BUFFER,
        MSS_SYS_NO_RESPONSE_LEN, mb_offset, MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_BITSTREAM_AUTHENTICATE_CMD,
...
MSS_SYS_authenticate_iap_image(uint32_t spi_idx) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_BITSTREAM_AUTHENTICATE_CMD, NULL_BUFFER,
        MSS_SYS_WITHOUT_CMD_DATA, NULL_BUFFER, MSS_SYS_NO_RESPONSE_LEN,
        (uint16_t)spi_idx, MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_BITSTREAM_AUTHENTICATE_CMD, NULL_BUFFER,
        MSS_SYS_WITHOUT_CMD_DATA, NULL_BUFFER, MSS_SYS_NO_RESPONSE_LEN,
...
MSS_SYS_digest_check(uint32_t options, uint8_t* digesterr, uint16_t mb_offset) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        ...
        (uint16_t)MSS_SYS_DIGEST_CHECK_SERVICE_RESP_LEN, mb_offset,
        MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_DIGEST_CHECK_CMD, (uint8_t*)&l_options,
...
MSS_SYS_execute_iap(uint8_t iap_cmd, uint32_t spiaddr) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)iap_cmd, (uint8_t*)&l_spiaddr, MSS_SYS_IAP_SERVICE_DATA_LEN,
        NULL_BUFFER, MSS_SYS_NO_RESPONSE_LEN, (uint16_t)spiaddr,
        MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)iap_cmd, (uint8_t*)&l_spiaddr, MSS_SYS_IAP_SERVICE_DATA_LEN,
...
MSS_SYS_spi_copy(
  ...
  *(uint32_t*)(mb_format + 12u) = n_bytes;
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_SPI_COPY_CMD, mb_format,
        (uint16_t)MSS_SYS_SPI_COPY_MAILBOX_DATA_LEN, NULL_BUFFER,
        MSS_SYS_NO_RESPONSE_LEN, mb_offset, MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_SPI_COPY_CMD, mb_format,
  ...
  *(uint16_t*)mb_format = service_data;
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        ...
        (uint16_t)MSS_SYS_PROBE_READ_SERVICE_RESP_LEN, mb_offset,
  ...
  *(uint32_t*)(mb_format + 8u) = pwdata;
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        ...
        (uint16_t)MSS_SYS_PROBE_WRITE_SERVICE_DATA_LEN, NULL_BUFFER,
        MSS_SYS_NO_RESPONSE_LEN, mb_offset, MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_PROBE_WRITE_DEBUG_CMD, mb_format,
...
MSS_SYS_debug_live_probe(
  ...
  *(uint32_t*)mb_format = service_data;
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        service_cmd, mb_format,
        (uint16_t)MSS_SYS_LIVE_PROBE_DEBUG_SERVICE_DATA_LEN, NULL_BUFFER,
        MSS_SYS_NO_RESPONSE_LEN, mb_offset, MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        service_cmd, mb_format,
...
MSS_SYS_debug_select_mem(
  ...
  *(uint16_t*)(mb_format + 4u) = timeout;
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_MEM_SELECT_DEBUG_CMD, mb_format,
        (uint16_t)MSS_SYS_MEM_SELECT_DATA_LEN, NULL_BUFFER,
        MSS_SYS_NO_RESPONSE_LEN, mb_offset, MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_MEM_SELECT_DEBUG_CMD, mb_format,
...
uint16_t
MSS_SYS_debug_read_mem(
    uint16_t mem_addr, uint16_t n_words, uint64_t mss_addr,
    uint16_t mb_offset) {
  uint16_t status       = MSS_SYS_PARAM_ERR;
  ...
  *(uint64_t*)(mb_format + 8u) = mss_addr;
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        ...
        (uint16_t)MSS_SYS_MEM_READ_WRITE_DATA_LEN, NULL_BUFFER,
        MSS_SYS_NO_RESPONSE_LEN, mb_offset, MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_MEM_READ_DEBUG_CMD, mb_format,
...
MSS_SYS_debug_read_apb(
  ...
  *(uint64_t*)(mb_format + 16u) = mss_addr;
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_APB_READ_DEBUG_CMD, mb_format,
        (uint16_t)MSS_SYS_APB_SERVICE_DATA_LEN, NULL_BUFFER,
        MSS_SYS_NO_RESPONSE_LEN, mb_offset, MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_APB_READ_DEBUG_CMD, mb_format,
...
MSS_SYS_debug_write_apb(
  ...
  *(uint64_t*)(mb_format + 16u) = mss_addr;
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_APB_WRITE_DEBUG_CMD, mb_format,
        (uint16_t)MSS_SYS_APB_SERVICE_DATA_LEN, NULL_BUFFER,
        MSS_SYS_NO_RESPONSE_LEN, mb_offset, MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
  ...
  *(uint32_t*)mb_format = port_addr;
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        ...
        (uint16_t)MSS_SYS_DEBUG_SNAPSHOT_DATA_LEN, NULL_BUFFER,
        MSS_SYS_NO_RESPONSE_LEN, mb_offset, MSS_SYS_COMMON_RET_OFFSET);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_DEBUG_SNAPSHOT_CMD, mb_format,
...
MSS_SYS_otp_generate(
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_GENERATE_OTP_CMD, mb_format,
        (uint16_t)MSS_SYS_GENERATE_OTP_DATA_LEN, n_fpga,
        (uint16_t)MSS_SYS_GENERATE_OTP_RESP_LEN, mb_offset, resp_offset);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_GENERATE_OTP_CMD, mb_format,
        (uint16_t)MSS_SYS_GENERATE_OTP_DATA_LEN, n_fpga,
...
MSS_SYS_otp_match(
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_MATCH_OTP_CMD, mb_format,
        (uint16_t)MSS_SYS_MATCH_OTP_DATA_LEN, NULL_BUFFER,
        MSS_SYS_NO_RESPONSE_LEN, mb_offset, resp_offset);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_MATCH_OTP_CMD, mb_format,
        (uint16_t)MSS_SYS_MATCH_OTP_DATA_LEN, NULL_BUFFER,
...
MSS_SYS_unlock_debug_passcode(
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        ...
        (uint16_t)MSS_SYS_UNLOCK_DEBUG_PASSCODE_DATA_LEN, NULL_BUFFER,
        MSS_SYS_NO_RESPONSE_LEN, mb_offset, resp_offset);
  } else {
    status = execute_ss_polling_mode(
        (uint8_t)MSS_SYS_UNLOCK_DEBUG_PASSCODE, cmd_data,
...
MSS_SYS_debug_terminate(uint16_t mb_offset, uint16_t resp_offset) {
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    status = execute_ss_interrupt_mode(
        (uint8_t)MSS_SYS_TERMINATE_DEBUG_CMD, NULL_BUFFER,
        MSS_SYS_WITHOUT_CMD_DATA, NULL_BUFFER, MSS_SYS_NO_RESPONSE_LEN,
...
MSS_SYS_read_response(void) {
  ...
  if (g_message_interrupt_counter > 0u) {
    ...
    if (g_int_service_response_size > 0u) {
      response_limit =
          g_int_service_response_size + g_int_service_response_offset;
  ...
  *MSS_SCBMESSAGE_INT = 0x0u; /*clear message_int reg*/
  ...
  if (MSS_SYS_SERVICE_INTERRUPT_MODE == g_service_mode) {
    MSS_SCBCTRL->SERVICES_CR =
        (((ss_command << SCBCTRL_SERVICESCR_COMMAND) &
...
/*
 * This function executes the SS command in interrupt mode. If Mailbox input
 * data is required by the service, the call to request_system_service()
 * function will first load it from cmd_data into the Mailbox. The response of
 * the service is not read by this function as it depends on message interrupt.
 * Application will have to read the response of service by calling
 * MSS_SYS_read_response(), only after interrupt occurs.
 */
static uint16_t
execute_ss_interrupt_mode(
    uint8_t cmd_opcode, uint8_t* cmd_data, uint16_t cmd_data_size,
  ...
  status = request_system_service(
      cmd_opcode, cmd_data, cmd_data_size, p_response, response_size, mb_offset,
      response_offset);
...
/***************************************************************************/ /**
                                                                               ...
                                                                               */
uint8_t
g5c_message_plic_IRQHandler(void) {
