...
context_switch_to_enclave(
    struct sbi_trap_regs* regs, enclave_id eid, int load_parameters) {
  /* save host context */
  ...
  if (load_parameters) {
    ...
    regs->mepc = (uintptr_t)enclaves[eid].params.runtime_entry -
...
context_switch_to_host(
    struct sbi_trap_regs* regs, enclave_id eid, int return_on_resume) {
  // set PMP
  int memid;
  for (memid = 0; memid < ENCLAVE_REGIONS_MAX; memid++) {
    if (enclaves[eid].regions[memid].type != REGION_INVALID) {
...
clean_enclave_memory(uintptr_t utbase, uintptr_t utsize) {
...
static unsigned long
encl_alloc_eid(enclave_id* _eid) {
  enclave_id eid;
  ...
  if (eid != ENCL_MAX) enclaves[eid].state = ALLOCATED;
...
int
get_enclave_region_index(enclave_id eid, enum enclave_region_type type) {
  size_t i;
  for (i = 0; i < ENCLAVE_REGIONS_MAX; i++) {
...
uintptr_t
get_enclave_region_size(enclave_id eid, int memid) {
...
uintptr_t
get_enclave_region_base(enclave_id eid, int memid) {
  if (0 <= memid && memid < ENCLAVE_REGIONS_MAX)
 ...
 */
unsigned long
copy_enclave_create_args(uintptr_t src, struct keystone_sbi_create* dest) {
  int region_overlap =
      copy_to_sm(dest, src, sizeof(struct keystone_sbi_create));
...
static unsigned long
copy_enclave_data(
    struct enclave* enclave, void* dest, uintptr_t source, size_t size) {
...
static unsigned long
copy_enclave_report(
    struct enclave* enclave, uintptr_t dest, struct report* source) {
...
static int
is_create_args_valid(struct keystone_sbi_create* args) {
 ...
 */
unsigned long
create_enclave(unsigned long* eidptr, struct keystone_sbi_create create_args) {
  ...
  if (encl_alloc_eid(&eid) != SBI_ERR_SM_ENCLAVE_SUCCESS) goto error;
  ...
  enclaves[eid].encl_satp =
      ((base >> RISCV_PGSHIFT) | (SATP_MODE_SV32 << HGATP_MODE_SHIFT));
#else
  enclaves[eid].encl_satp =
      ((base >> RISCV_PGSHIFT) | (SATP_MODE_SV39 << HGATP_MODE_SHIFT));
#endif
 ...
 */
unsigned long
destroy_enclave(enclave_id eid) {
  int destroyable;
  ...
  if (destroyable) enclaves[eid].state = DESTROYING;
  spin_unlock(&encl_lock);

  if (!destroyable) return SBI_ERR_SM_ENCLAVE_NOT_DESTROYABLE;
  ...
  if (rid != -1) pmp_region_free_atomic(enclaves[eid].regions[rid].pmp_rid);
...
unsigned long
run_enclave(struct sbi_trap_regs* regs, enclave_id eid) {
  int runable;
...
unsigned long
exit_enclave(struct sbi_trap_regs* regs, enclave_id eid) {
  int exitable;
  ...
  if (exitable) {
    enclaves[eid].n_thread--;
    if (enclaves[eid].n_thread == 0) enclaves[eid].state = STOPPED;
  }
  spin_unlock(&encl_lock);

  if (!exitable) return SBI_ERR_SM_ENCLAVE_NOT_RUNNING;
...
unsigned long
stop_enclave(struct sbi_trap_regs* regs, uint64_t request, enclave_id eid) {
  int stoppable;
  ...
  if (!stoppable) return SBI_ERR_SM_ENCLAVE_NOT_RUNNING;
...
unsigned long
resume_enclave(struct sbi_trap_regs* regs, enclave_id eid) {
  int resumable;
...
unsigned long
attest_enclave(
    uintptr_t report_ptr, uintptr_t data, uintptr_t size, enclave_id eid) {
  int attestable;
  struct report report;
  int ret;

  if (size > ATTEST_DATA_MAXLEN) return SBI_ERR_SM_ENCLAVE_ILLEGAL_ARGUMENT;
  ...
  sm_sign(
      ...
      sizeof(struct enclave_report) - SIGNATURE_SIZE - ATTEST_DATA_MAXLEN +
          size);
...
unsigned long
get_sealing_key(
    uintptr_t sealing_key, uintptr_t key_ident, size_t key_ident_size,
    enclave_id eid) {
  ...
  ret = sm_derive_sealing_key(
      (unsigned char*)key_struct->key, (const unsigned char*)key_ident,
      key_ident_size, (const unsigned char*)enclaves[eid].hash);
  if (ret) return SBI_ERR_SM_ENCLAVE_UNKNOWN_ERROR;

  /* sign derived key */
  sm_sign(
      (void*)key_struct->signature, (void*)key_struct->key, SEALING_KEY_SIZE);
