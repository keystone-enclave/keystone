...
static void
sbi_trap_error(
    const char* msg, int rc, ulong mcause, ulong mtval, ulong mtval2,
 ...
 */
void
sbi_trap_handler_keystone_enclave(struct sbi_trap_regs* regs) {
  int rc          = SBI_ENOTSUPP;
...
trap_error:
  if (rc) sbi_trap_error(msg, rc, mcause, mtval, mtval2, mtinst, regs);
}
