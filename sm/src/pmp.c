...
static inline int
region_register_idx(region_id i) {
  return regions[i].reg_idx;
}

static inline int
region_allows_overlap(region_id i) {
  return regions[i].allow_overlap;
}

static inline uintptr_t
region_get_addr(region_id i) {
  return regions[i].addr;
}

static inline uint64_t
region_get_size(region_id i) {
  return regions[i].size;
}

static inline int
region_is_napot(region_id i) {
  return regions[i].addrmode == PMP_A_NAPOT;
}

static inline int
region_is_tor(region_id i) {
  return regions[i].addrmode == PMP_A_TOR;
}

static inline int
region_needs_two_entries(region_id i) {
  return region_is_tor(i) && regions[i].reg_idx > 0;
}

static inline int
region_is_napot_all(region_id i) {
  return regions[i].addr == 0 && regions[i].size == -1UL;
}

static inline uintptr_t
region_pmpaddr_val(region_id i) {
...
static inline uintptr_t
region_pmpcfg_val(region_id i, pmpreg_id reg_idx, uint8_t perm_bits) {
  return (uintptr_t)(regions[i].addrmode | perm_bits)
         << (8 * (reg_idx % PMP_PER_GROUP));
}

static void
region_clear_all(region_id i) {
  regions[i].addr          = 0;
  regions[i].size          = 0;
...
static void
region_init(
    region_id i, uintptr_t addr, uint64_t size, uint8_t addrmode,
    int allow_overlap, pmpreg_id reg_idx) {
  regions[i].addr          = addr;
  regions[i].size          = size;
  regions[i].addrmode      = addrmode;
  regions[i].allow_overlap = allow_overlap;
  regions[i].reg_idx =
      (addrmode == PMP_A_TOR && reg_idx > 0 ? reg_idx + 1 : reg_idx);
}

static int
is_pmp_region_valid(region_id region_idx) {
  return TEST_BIT(region_def_bitmap, region_idx);
}

static int
search_rightmost_unset(uint32_t bitmap, int max, uint32_t mask) {
  int i = 0;
 ...
 */
static int
detect_region_overlap(uintptr_t addr, uintptr_t size) {
  void* epm_base;
  size_t epm_size;
  ...
  for (i = 0; i < PMP_MAX_N_REGION; i++) {
    if (!is_pmp_region_valid(i)) continue;
...
int
pmp_detect_region_overlap_atomic(uintptr_t addr, uintptr_t size) {
...
int
pmp_unset_global(int region_idx) {
...
int
pmp_set_global(int region_idx, uint8_t perm) {
  if (!is_pmp_region_valid(region_idx))
    PMP_ERROR(SBI_ERR_SM_PMP_REGION_INVALID, "Invalid PMP region index");
...
int
pmp_set_keystone(int region_idx, uint8_t perm) {
  if (!is_pmp_region_valid(region_idx))
    PMP_ERROR(SBI_ERR_SM_PMP_REGION_INVALID, "Invalid PMP region index");
  ...
  // sbi_printf("pmp_set() [hart %d]: reg[%d], mode[%s], range[0x%lx-0x%lx],
  // perm[0x%x]\r\n",
  //        current_hartid(), reg_idx, (region_is_tor(region_idx) ?
  //        "TOR":"NAPOT"), region_get_addr(region_idx),
  //        region_get_addr(region_idx) + region_get_size(region_idx), perm);
...
int
pmp_region_init_atomic(
    uintptr_t start, uint64_t size, enum pmp_priority priority, region_id* rid,
    int allow_overlap) {
  int ret;
  spin_lock(&pmp_lock);
...
tor_region_init(
  ...
  if (region_idx < 0 || region_idx > PMP_MAX_N_REGION)
    PMP_ERROR(
        SBI_ERR_SM_PMP_REGION_MAX_REACHED,
        "Reached the maximum number of PMP regions");
  ...
  *rid = region_idx;
  switch (priority) {
    case (PMP_PRI_ANY): {
      ...
      if (reg_idx < 0)
        PMP_ERROR(
            SBI_ERR_SM_PMP_REGION_MAX_REACHED, "No available PMP register");
      if (TEST_BIT(reg_bitmap, reg_idx) || TEST_BIT(reg_bitmap, reg_idx + 1) ||
          reg_idx + 1 >= PMP_N_REG)
        PMP_ERROR(
            SBI_ERR_SM_PMP_REGION_MAX_REACHED, "PMP register unavailable");
  ...
  if (reg_idx > 0) SET_BIT(reg_bitmap, reg_idx + 1);
...
int
pmp_region_free_atomic(int region_idx) {
  spin_lock(&pmp_lock);
  ...
  if (region_needs_two_entries(region_idx)) UNSET_BIT(reg_bitmap, reg_idx - 1);
...
int
pmp_region_init(
    uintptr_t start, uint64_t size, enum pmp_priority priority, int* rid,
    int allow_overlap) {
  if (!size) PMP_ERROR(SBI_ERR_SM_PMP_REGION_SIZE_INVALID, "Invalid PMP size");
  ...
  if (size != -1UL && (size & (RISCV_PGSIZE - 1)))
    PMP_ERROR(
        SBI_ERR_SM_PMP_REGION_NOT_PAGE_GRANULARITY,
        "PMP granularity is RISCV_PGSIZE");
...
uintptr_t
pmp_region_get_addr(region_id i) {
  if (is_pmp_region_valid(i)) return region_get_addr(i);
  return 0;
}

uint64_t
pmp_region_get_size(region_id i) {
  if (is_pmp_region_valid(i)) return region_get_size(i);
  return 0;
}
