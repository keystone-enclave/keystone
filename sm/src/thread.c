...
void
swap_prev_mstatus(
    struct thread_state* thread, struct sbi_trap_regs* regs,
    uintptr_t current_mstatus) {
  ...
  uintptr_t mstatus_mask = MSTATUS_SIE | MSTATUS_SPIE | MSTATUS_SPP |
                           MSTATUS_MPP | MSTATUS_FS | MSTATUS_SUM | MSTATUS_MXR;

  uintptr_t tmp = thread->prev_mstatus;
  thread->prev_mstatus =
      (current_mstatus & ~mstatus_mask) | (current_mstatus & mstatus_mask);
...
void
swap_prev_smode_csrs(struct thread_state* thread) {
  uintptr_t tmp;
...
void
clean_state(struct thread_state* state) {
  int i;
...
void
clean_smode_csrs(struct thread_state* state) {
