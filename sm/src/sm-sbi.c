...
unsigned long
sbi_sm_create_enclave(unsigned long* eid, uintptr_t create_args) {
...
sbi_sm_get_sealing_key(
  ...
  unsigned long ret;
  ret = get_sealing_key(
      sealing_key, key_ident, key_ident_size, cpu_get_enclave_id());
  return ret;
}
...
unsigned long
sbi_sm_call_plugin(
    uintptr_t plugin_id, uintptr_t call_id, uintptr_t arg0, uintptr_t arg1) {
  unsigned long ret;
