...
uintptr_t
multimem_get_other_region_size(enclave_id eid, size_t* size_out) {
...
uintptr_t
multimem_get_other_region_addr(enclave_id eid, size_t* size_out) {
  int mem_id = get_enclave_region_index(eid, REGION_OTHER);
  if (mem_id == -1) return -1;
...
uintptr_t
do_sbi_multimem(enclave_id eid, uintptr_t call_id, uintptr_t arg0) {
  switch (call_id) {
