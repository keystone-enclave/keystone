...
waymask_allocate_ways(
    size_t n_ways, unsigned int target_hart, waymask_t* mask) {
  *mask            = 0;
...
int
master_is_for_hart(unsigned int master, unsigned int hart) {
...
_wm_choose_ways_for_hart(
    size_t n_ways, waymask_t* mask, unsigned int target_hart) {
  *mask = 0;
...
int
_wm_grant_ways(waymask_t mask, unsigned int master) {
  if (master > WM_NUM_MASTERS) {
    return -1;
  }
