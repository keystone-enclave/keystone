...
scratch_init() {
  ...
  */
  ...
  for (tmp_mask = 0x80; tmp_mask <= scratchpad_allocated_ways;
       tmp_mask = tmp_mask << 1) {
...
platform_init_global_once() {
  ...
  if (pmp_region_init_atomic(
          CACHE_CONTROLLER_ADDR_START,
          CACHE_CONTROLLER_ADDR_END - CACHE_CONTROLLER_ADDR_START, PMP_PRI_ANY,
          &l2_controller_rid, 1)) {
  ...
  if (pmp_region_init_atomic(
          L2_SCRATCH_START, L2_SCRATCH_STOP - L2_SCRATCH_START, PMP_PRI_ANY,
          &scratch_rid, 1)) {
...
void
platform_init_enclave(struct enclave* enclave) {
...
unsigned long
platform_create_enclave(struct enclave* enclave) {
  enclave->ped.use_scratch = 0;
  if (enclave->ped.use_scratch) {
    ...
    uintptr_t old_epm_start =
        pmp_region_get_addr(enclave->regions[old_epm_idx].pmp_rid);
    ...
    memcpy(
        (unsigned long*)scratch_epm_start, (unsigned long*)old_epm_start, size);
    sbi_printf(
        "Performing copy from %lx to %lx\r\n", old_epm_start,
        scratch_epm_start);
    ...
    enclave->encl_satp =
        ((scratch_epm_start >> RISCV_PGSHIFT) |
         (SATP_MODE_SV39 << HGATP_MODE_SHIFT));
...
void
platform_destroy_enclave(struct enclave* enclave) {
  if (enclave->ped.use_scratch) {
    ...
    uintptr_t scratch_start =
        pmp_region_get_addr(enclave->regions[scratch_epm_idx].pmp_rid);
    uintptr_t scratch_stop =
        scratch_start +
        pmp_region_get_size(enclave->regions[scratch_epm_idx].pmp_rid);
...
void
platform_switch_to_enclave(struct enclave* enclave) {
  if (enclave->ped.num_ways > 0) {
    // Each hart gets special access to some
    unsigned int core = csr_read(mhartid);

    // Allocate ways, fresh every time we enter
    waymask_allocate_ways(
        enclave->ped.num_ways, core, &enclave->ped.saved_mask);
...
void
platform_switch_from_enclave(struct enclave* enclave) {
  if (enclave->ped.num_ways > 0) {
    /* Free all our ways */
