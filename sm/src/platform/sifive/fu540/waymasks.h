...
#define WM_MAX_FREE_WAY \
  WM_NUM_WAYS - 2 /* We allocate 1 way for all non hart masters */
...
#define WM_REG_ADDR(id) \
  ((waymask_t*)(CACHE_CONTROLLER_ADDR_START + WM_REG_OFFSET(id)))
...
/* Interface */
size_t
waymask_allocate_ways(size_t n_ways, unsigned int target_hart, waymask_t* mask);
void
waymask_apply_allocated_mask(waymask_t mask, unsigned int target_hart);
void
waymask_free_ways(waymask_t _mask);
void
waymask_init();
void
waymask_clear_ways(waymask_t mask, unsigned int core);
...
/* Internals */
int
_wm_choose_ways_for_hart(
    size_t n_ways, waymask_t* _mask, unsigned int target_hart);
int
_wm_lockout_ways(waymask_t mask, unsigned int master);
int
_wm_grant_ways(waymask_t mask, unsigned int hart);
int
_wm_assign_mask(waymask_t mask, unsigned int master);
void
